...
func TestBatchGet_AllHit(t *testing.T) {
	...
	// 预热
	_, err := BatchGet(context.Background(), rds, []int64{1, 2}, loader.load)
	require.NoError(t, err)
	require.Equal(t, 1, loader.calls)

	// 二次全命中 不再回源
	res, err := BatchGet(context.Background(), rds, []int64{1, 2}, loader.load)
	require.NoError(t, err)
...
func TestBatchGet_PartialHit(t *testing.T) {
	...
	// 只热 content 1
	_, err := BatchGet(context.Background(), rds, []int64{1}, loader.load)
	require.NoError(t, err)
	loader.calls = 0
...
func TestBatchGet_DedupAndSkipNonPositive(t *testing.T) {
	...
	res, err := BatchGet(context.Background(), rds, []int64{1, 2, 3, 3, 0, -1}, loader.load)
	require.NoError(t, err)
	assert.Equal(t, 1, loader.calls)
	assert.Len(t, res, 3)
...
func TestBatchGet_MissThenFill(t *testing.T) {
	...
	res, err := BatchGet(context.Background(), rds, []int64{42}, loader.load)
	require.NoError(t, err)
...
func TestBatchGet_NotFoundWritesSentinel(t *testing.T) {
	...
	res, err := BatchGet(context.Background(), rds, []int64{999}, loader.load)
	require.NoError(t, err)
	assert.Nil(t, res[999])

	// 负哨兵写入 negative TTL 加抖动
